...
def get_dea_images(layername, year, bbox, resolution, outpath, crs = 'EPSG:4326', format_out = 'GeoTIFF'):
    ...
    for date in dates:
        ...
        download_ok = get_wcsmap(outfname, layername, bbox, date, resolution, url, crs = crs, format_out = format_out)
        if download_ok:
            print(f'{layername} for date {date} downloaded')
        else:
            print(f'{layername} for date {date} failed to download')
            return False
...
def test_get_dea_images():
    ...
    outpath = 'test_dea'
    # Get data
    download_ok = get_dea_images(layername, year, bbox, resolution, outpath, crs = crs)
    assert download_ok
